...
import { getAllUsersList, inactiveUser } from "../../index.js";
...
function Users() {
  ...
  const getUsersToSet = useCallback(
  ...
  useEffect(() => {
    getUsersToSet(searchTerm, currentPage, { showMainLoader: true });
  }, [currentPage, searchTerm, getUsersToSet]);

  // Debounce estable para búsqueda
  const debouncedSearch = useMemo(
    () =>
      debounce((term) => {
        getUsersToSet(term, 1);
        setCurrentPage(1);
      }, 800),
    [getUsersToSet]
  );
  ...
  const handlePreviousPage = () => {
    if (currentPage > 1) {
      getUsersToSet(searchTerm, currentPage - 1, { showMainLoader: true });
  ...
  // Inactivar
  function handleInactiveUser(userId) {
    setOpenModalToDelete(true);
    setUserToInactive(userId);
  }
  function inactiveSingleUser(userId) {
    inactiveUser(userId)
      .then(() => {
        getUsersToSet(searchTerm, currentPage, { showMainLoader: true });
      })
    ...
    <>
      <div className="pb-8 px-4 sm:px-8 lg:px-16 bg-gray-100 min-h-screen">
        <div className="flex flex-wrap gap-4 justify-between items-center mb-8 bg-gray-800 p-4 sm:p-8 rounded-bl-2xl rounded-br-2xl border-b-2 border-b-emerald-500 border-l-2 border-l-emerald-500 border-r-2 border-r-emerald-500 shadow-sm">
          <h1 className="text-2xl sm:text-4xl font-semibold text-white">Usuarios</h1>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <p className="m-0 leading-loose">Crear Usuario</p>
            </Link>
          </div>
        </div>
                  ...
                  <tr key={user._id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div className="flex gap-2">
                        {/* Solo mostrar "Editar" para Admins o cuando el usuario listado NO es Admin */}
                        {(loggedUser?.role === 1 || user.role !== 1) && (
                          <Link
                        ...
                        {/* Inactivar: solo admins, como ya teníamos */}
                        {loggedUser?.role === 1 && (
                          <button
                            onClick={() => handleInactiveUser(user._id)}
                          ...
                          >
                            <img
                              ...
                              className="w-[18px]"
                            />
                            <p className="m-0 leading-loose">Inactivar</p>
                          </button>
                        )}
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-10 rounded-lg shadow-lg flex justify-center items-center flex-col">
            <h2 className="text-xl mb-4">
              ¿Seguro que desea inactivar el usuario?
            </h2>
            <div className="flex gap-4">
              <button
                className="bg-red-500 text-white py-2 px-4 rounded"
                onClick={() => inactiveSingleUser(userToInactive)}
              >
                Inactivar
              </button>
              <button
